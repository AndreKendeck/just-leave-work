 ...
 */
const Dropdown = ({ options, errors, hasError, selectedOption, name, label }) => {
    ...
    return (
        <div className="flex flex-col space-y-1">
            ...
            <select name={name} id={name} className={`form-select border-2  rounded-lg ${hasError ? 'border-red-500' : 'border-gray-300'}`}>
                {mapOptions(options)}
            </select>
            <div className="flex flex-col space-y-1">
                {getErrors(errors)}
